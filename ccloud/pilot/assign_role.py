...
from dracclient.client import DRACClient  # noqa
...
def parse_arguments():
    ...
    parser.add_argument("-f",
                        "--flavor-settings",
                        default="~/ccloud/pilot/flavors_settings.json",
...
def assign_role(ip_mac_service_tag, node_uuid, role_index, os_volume_size_gb,
    ...
    #LOG.info("RapidCompute Log - Skipping select_os_volume function")
    #print("RapidCompute Log - Skipping select_os_volume function")
    LOG.info("***---RapidCompute Log - Starting select_os_volume function---***")
    print("***---RapidCompute Log - Starting select_os_volume function---***")
    select_os_volume(os_volume_size_gb, ironic_client, drac_client,
                     node_uuid)
    LOG.info("***---RapidCompute Log - Stopping select_os_volume function---***")
    print("***---RapidCompute Log - Stopping select_os_volume function---***")

    # Generate Ceph OSD/journal configuration for storage nodes
    if flavor == "ceph-storage":
        #LOG.info("Skipping osd_config function")
        #print("Skipping osd_config function")
        LOG.info("Starting osd_config function")
        print("Starting osd_config function")
        generate_osd_config(ip_mac_service_tag, drac_client)
        LOG.info("Finishing off osd_config function")
        print("Finishing off osd_config function")
